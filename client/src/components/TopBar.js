...
export default function TopBar({ watchlistData, setWatchlistData }) {
  ...
  function handleSearch(symbol) {
    if (
      symbol !== "" &&
      !watchlistData.some((item) => item.symbol === symbol)
    ) {
      axios
        .post(`${url}api/postsymbol/sendsymbol`, { symbol })
        .then((res) => {
  ...
  function updateResult(userInput) {
    const results = combinedFull.filter((item) => {
      return (
        item.symbol.includes(userInput.toUpperCase()) ||
          ...
          <div className="search-container">
            <div className="control-search">
              <input
                ...
                name="search"
                autoComplete="off"
                placeholder="Search Stock In Watchlist..."
                ref={inputRef}
                onChange={(e) => {
